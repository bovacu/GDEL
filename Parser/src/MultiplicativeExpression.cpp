...
json MultiplicativeExpression::getAst(const Expression& _expression, Parser* _parser, json& _tokenToCheck) const {
    auto _left = this->rangeExpression.getAst(_expression, _parser, _parser->getLookAhead());
    ...
    while ( strcmp(_parser->getCurrentLookAheadType().c_str(), _MUL) == 0 || 
        ...
        auto _right = this->rangeExpression.getAst(_expression, _parser, _parser->getLookAhead());
        _left = {
            {"type", _BINARY_OP},
            {"operator", _op},
            {"left", _left},
