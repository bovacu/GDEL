...
json ForLoopStatement::getAst(const Statement& _statement, Parser& _parser, json& _tokenToCheck) const {
    ...
    std::vector<json> _indexVar = _variableStatement.getDeclarationList(_statement, _statement.getExpressionStatement().getExpression(), 
    _parser, _parser.getLookAhead(), true);
