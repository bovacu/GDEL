...
json RelationalExpression::getAst(const Statement& _statement, Parser* _parser, json& _tokenToCheck) const {
    auto _left = this->additiveExpression.getAst(_statement, _parser, _parser->getLookAhead());
    ...
    while ( strcmp(_parser->getCurrentLookAheadType().c_str(), _COM_GT) == 0 ||
        ...
        auto _right = this->additiveExpression.getAst(_statement, _parser, _parser->getLookAhead());
        _left = {
            {"type", _BINARY_OP},
            {"operator", _op},
            {"left", _left},
